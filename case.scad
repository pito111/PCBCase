...
module case(width=20, // Main PCB width
		length=20,	// Main PCB height
		base=2.5,	// Base below PCB
		top=2.5,	// Top above PCB
		side=3,		// Side around PCB
		sidet=0.15,	// Tolerance for size to fit - depends a bit on printer
		margin=0.3,	// Margin around PCB and parts x/y - depends a bit on printer
		...
		sideedge=1	// Corner cut off sides
		)
{
	// Base
	intersection()
...
module casebox(width,length,base,top,side,sidet,margin,pcb,baseedge,topedge,sideedge)
{ // The box
	difference()
	{
		hull()
