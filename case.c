...
void write_scad(void)
{
   ...
   *o3;
   ...
   int find_module(char **fnp, const char *a, const char *b) {  // Find a module by filename, and return number, <0 for failed (including file not existing)
      char *fn = *fnp;
      *fnp = NULL;
      int n;
      for (n = 0; n < modulen; n++)
         ...
         {                      // Consider parameterised maybe?
            ...
            if (*p)
            {
               ...
               while (*p && !isdigit(*p))
                  p++;
               if (*p)
                  index = 0;    // More than one number in place.
            }
            if (index)
            {                   // Make filename
               if (asprintf(&fn, "%s.scad", r) < 0)
